...
Write-Host "电脑地址：http://localhost:$port/?v=11" -ForegroundColor Green
if ($lanIp) {
  Write-Host "华为 Pad 从第 9 关继续：http://${lanIp}:$port/?v=11&resume=9" -ForegroundColor Yellow
  Write-Host '请确保电脑和平板连接同一个家庭路由器。' -ForegroundColor DarkGray
}
Start-Process "http://localhost:$port/?v=11"
